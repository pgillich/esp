...
public class Body implements Steppable {
	...
	// Front/rear	
		...
		MutableDouble2D F_cp_ABCD = state.simpleBeam(F_cp, state.l_1, state.l_2);
		...
		MutableDouble2D G_ABCD = state.simpleBeam(G, state.l_1, state.l_2);	
	...
	// Front	
		...
		MutableDouble2D G_AB_ = state.simpleBeam(G_AB, state.w_1, state.w_2);
		double G_A = G_AB_.x;
		double G_B = G_AB_.y;		
		...
		double F_B_x = Math.min(F_B_x_1, F_s_B_max);
		...
		double F_A_x = Math.min(F_A_x_1, F_s_A_max);
	...
	// Rear	
		...
		MutableDouble2D G_DC_ = state.simpleBeam(G_DC, state.w_1, state.w_2);
		double G_D = G_DC_.x;
		double G_C = G_DC_.y;
		...
		double F_C_x = Math.min(F_C_x_1, F_s_C_max);
		...
		double F_D_x = Math.min(F_D_x_1, F_s_D_max);
	...
	// a, torque, v, location	
		...
		if( state.schedule.getSteps() % state.printStep == 0 )
		{
			...
			Transform3D ft;
/*			
			state.labels.setObjectLocation(state.forceMap.get("A"), new Double3D(location.x+1, location.y+1, 0));
			
			ft = new Transform3D();
			ft.setTranslation(new Vector3d(F_A_x,0,G_A));
			ft.rotZ(alpha-Esp.QUADRANT);
			ft.setScale(1E-3);
			state.forceMap.get("A").toNodePortrayal.setTransform(ft);
*/			
			for (BodyForce bf : state.forceMap.values()) 
			{
				...
				Double3D toNodeXLocation = bf.transformToNodeLocation(fromNodeLocation, alpha-Esp.QUADRANT, 1, 0, 0);
				state.labels.setObjectLocation(bf.toNodeXObject,toNodeXLocation);

				Double3D toNodeYLocation = bf.transformToNodeLocation(fromNodeLocation, alpha-Esp.QUADRANT, 0, 1, 0);
				state.labels.setObjectLocation(bf.toNodeYObject,toNodeYLocation);

				Double3D toNodeZLocation = bf.transformToNodeLocation(fromNodeLocation, alpha-Esp.QUADRANT, 0, 0, 1);
				state.labels.setObjectLocation(bf.toNodeZObject,toNodeZLocation);
			}

		}
		
		if(state.schedule.getTime() > 15.)
		{
			state.kill();
		}
	}
