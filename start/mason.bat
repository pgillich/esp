...
SET PATH=%STARTING_POINT%;%MASON_DIR%\bins;%PATH%
...
call ignoreme.bat %MASON_DIR%\bin
...
(for /F %%f IN ('dir /b /a-d "%JAR_DIR%\jars\*.jar"') do call ignoreme.bat %JAR_DIR%\jars\%%f%) 2>nul

SET

rem java -Xmx200M sim.display.Console >nul 2>nul 
java -Xmx200M -Xdiag sim.app.esp.EspWithUI
